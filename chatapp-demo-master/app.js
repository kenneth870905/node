...
socketIO.on('connection', function (socket) {
    ...
    socket.on('add', function ([userName,roomID01]) {
        console.log('add',userName,roomID01)
        roomID = roomID01
        user = userName;
        // 将用户昵称加入房间名单中
        if (!roomInfo[roomID]) {
    ...
    socket.on('disconnect', function () {
        // 从房间名单中移除
        console.log(socket.rooms)
        console.log('从房间名单中移除',roomInfo,roomID,user)
    ...
    socket.on('message', function (msg) {
        // 验证如果用户不在房间内则不给发送
        console.log('接收用户消息,发送相应的房间',roomInfo,roomID,user)
        console.log(socket.rooms)
